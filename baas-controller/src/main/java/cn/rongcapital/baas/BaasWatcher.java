	...
	public Mono<Boolean> watch(String name, String version) {
		...
		Mono<Boolean> result = Mono.create(callback -> {
			...
			try {
				...
				checkResult.publishOn(Schedulers.parallel()).subscribe(running -> {
					if (running) {
						logger.info("k8s代理检测结果为running场景");
						callback.success(true);
					} else {
						logger.info("k8s代理检测结果为other场景");
						callback.error(new RuntimeException("container state is "));
					}
				}, error -> {
		...
		result = result.timeout(Duration.ofMillis(DurationDefinition.CONTROLLER_TIMEOUT_MS)).delayElement(Duration.ofMillis(DurationDefinition.CONTROLLER_DELAY_MS)).retry(200);
		return result;
	}
