...
public class BaasAgent {
	...
	public Mono<Boolean> pull(String name, String version) {
			...
			findResult.publishOn(Schedulers.parallel()).subscribe(value -> {
				try {
					...
					if (0 == replicas) {
						...
						specMap.put("replicas", 1);
						logger.info("PUT方式调用k8s deployments api, 设置副本集数量为1");
						...
						logger.info("执行k8s deployments api返回结果中的副本集数量为1场景");
						callback.success(true);
					}
				} catch (Exception e) {
		...
		Mono<Boolean> mono = Mono.create(callback -> {
			...
			result.publishOn(Schedulers.parallel()).subscribe(value -> {
				try {
					...
					List<Object> states = JsonPath.read(value, "$.items[*].status.containerStatuses[?(@.name == 'baas-" + name + "')].state.running");
					if (CollectionUtils.isEmpty(states)) {
						logger.info("执行k8s pods api结果中state不存在running场景");
						callback.success(false);
					} else {
						logger.info("执行k8s pods api结果中state存在running场景");
						callback.success(true);
					}
				} catch (Exception e) {
